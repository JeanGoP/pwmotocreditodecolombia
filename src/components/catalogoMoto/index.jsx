...
const CatalogoMoto = ({ ColorTitulo, colorPagina }) => {
  ...
  const cilindrajesUnicos = useMemo(() => {
    return [...new Set(productos.map((p) => p.ficha_tecnica?.find((i) => i.ficha_tecnica_name === 'Cilindrada')?.ficha_tecnica_detalle).filter(Boolean))];
  }, [productos]);
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="text-end container mb-3">
        ...
        <button
          className="btn btn-primary btnfiltros"
          style={{ backgroundColor: '#004260', padding: '15px', borderRadius: '15px' }}
      ...
      {productosParaMostrar.length > 0 && productosParaMostrar.length < productosFiltrados.length && (
        <div className="text-center mt-4">
          <button className="btn btn-outline-primary btn__mostrarMas" style={{ backgroundColor: '#004260' }} onClick={cargarMas}>
              ...
              >
                              ...
                              <div className="" style={{ paddingTop: '15px' }}>
                                ...
                                <div className="motoObjeto-colores">
                                  {productoSeleccionado.color.length > 5 && (
                                    <>
                                      <button className="carousel-btn left" onClick={() => scrollColores('left')}>
                                        ‹
                                      </button>
                                      <button className="carousel-btn right" onClick={() => scrollColores('right')}>
                                        ›
                                      </button>
                                    </>
                                  )}
                                    ...
                                    {productoSeleccionado.color.map((c, i) => (
                                      ...
                                      >
                                        <span
                                          ...
                                          style={{
                                            background: c.color_code?.startsWith('linear') ? c.color_code : c.color_code || '#fff',
                                          }}
                                        />
                                      </button>
                                    ))}
                            ...
                            <div className="motoObjeto-precio text-center">
                              <h4>
                                {(() => {
                                  ...
                                  if (añosDisponibles.length === 0) return 'Precio No Disponible';
                              ...
                              <p>Precio • No incluye matrícula, gastos administrativos, ni SOAT!</p>
                            </div>
                          </div>
                          <div className="d-flex justify-content-center align-items-center">
                            <div className="motoObjeto-botones">
                              <button className="btn btn-rojo--contac" onClick={() => handleCotizacion()}>
                                Quiero Esta Moto!
                              </button>
                            </div>
                          </div>
                  ...
                  {productoSeleccionado?.ficha_tecnica?.length > 0 && (
                    <div className="mb-4" style={{ paddingTop: '50px' }}>
                      <div style={{ position: 'relative' }}>
                        {mostrarFlechas && (
                          <>
                            <button className="carousel-btn leftf" onClick={() => scrollFicha('left')}>
                              ‹
                            </button>

                            <button className="carousel-btn rightf" onClick={() => scrollFicha('right')}>
                              ›
                            </button>
                          </>
                        )}
                        ...
                        >
                          {productoSeleccionado.ficha_tecnica.map((item, idx) => {
                            const icono = imagenLogo.find((img) => img.nombre.trim().toLowerCase() === item.ficha_tecnica_name.trim().toLowerCase()) || {
                            ...
                            return (
                              <div className="ficha-item" key={idx}>
                                <CardDetalle
                                  ...
                                  contenido={item.ficha_tecnica_detalle ? item.ficha_tecnica_detalle.toLowerCase().replace(/^\w/, (c) => c.toUpperCase()) : ''}
                                  icono={icono.ruta}
                                />
                              </div>
                            );
                ...
                <div className="modal-footer" style={{ borderTop: '1px solid #dee2e6' }}>
                  <button
                    ...
                    style={{ background: '#004260', color: 'white', border: 'none' }}
                  >
                    Cerrar
                  </button>
                </div>
