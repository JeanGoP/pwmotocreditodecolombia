...
const Navbar = ({ onAbrilModal }) => {
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-dark d-flex justify-content-center nav__menu">
      <div className="container d-flex justify-content-between align-items-center nav_submen">
              ...
              item.submenu ? (
                ...
                <li className="nav-item" key={index}>
                  ...
                  <NavLink
                    to={item.ruta ? `/${item.ruta}` : '#'}
                    onClick={() => handleClick(item.label)}
                    className={({ isActive }) => `nav-link stylish-nav-link ${isActive ? 'nav-active' : ''}`}
                  >
                    {item.label}
                  </NavLink>
                </li>
